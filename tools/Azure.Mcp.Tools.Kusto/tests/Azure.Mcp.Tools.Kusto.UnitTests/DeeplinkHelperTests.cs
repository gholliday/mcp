...
public sealed class DeeplinkHelperTests
{
    ...
    [Fact]
    public void BuildWebExplorerUrl_DatabaseNameWithSpecialCharacters_EncodesCorrectly()
    {
        // Arrange - database names can contain spaces, hyphens, and periods
        var clusterUri = "https://help.kusto.windows.net";
        var database = "My Database.2024-01";
        var query = "T | take 1";
        ...
        // Assert
        Assert.NotNull(url);
        Assert.Contains("/databases/My%20Database.2024-01?query=", url);
    }
